...
{
    public class GreenLightService : WebSocketService, IServicePlus
    {
        ...
        public GreenLightService(WebSocketServiceManager _manager, IWebSocketConnection socket)
        {
        ...
        public void FMSend(command _cmd)
        {
            ...
            if (last_command != null)
            {
                if (last_command.Name == _cmd.Name)
                {
                    ...
                    {
                        case stateName.打开:
                            Debug.WriteLine(string.Format("{0} 打开绿灯", last_command.Commander));
                            _cmd.Para = string.Format("{0}打开了绿灯", last_command.Commander);
                            last_effective_command = _cmd;
                            recently_broadcast = JsonConvert.SerializeObject(_cmd);
                            this.Broadcast(recently_broadcast);
                            break;
                        case stateName.关闭:
                            Debug.WriteLine(string.Format("{0} 关闭绿灯", last_command.Commander));
                            _cmd.Para = string.Format("{0}关闭了绿灯", last_command.Commander);
                            last_effective_command = _cmd;
                            recently_broadcast = JsonConvert.SerializeObject(_cmd);
                            this.Broadcast(recently_broadcast);
                            break;
        ...
        public void MCOnMessage(command _cmd)
        {
            ...
            Debug.WriteLine(string.Format("GreenLightService OnMessage => {0}", msg));
        ...
        public override void OnMessage(string msg)
        {
            ...
            //            Debug.WriteLine(string.Format("{0} 试图打开绿灯", cmd.Commander));
            //            break;
            //        case "close":
            //            关闭灯(Program.getRemoteIPEndPoint());
            //            Debug.WriteLine(string.Format("{0} 关闭绿灯", cmd.Commander));
            //            break;
            //    }
        ...
        //                               recently_broadcast = JsonConvert.SerializeObject(ncOpen);
        //                               this.Send(recently_broadcast);//只通知本人
        //                               Debug.WriteLine("关闭绿灯操作失败");
        //                               break;
        //                           case false:
        //                               command ncClose = new command("close", "操作失败");
        //                               recently_broadcast = JsonConvert.SerializeObject(ncClose);
        //                               this.Send(recently_broadcast);
        //                               Debug.WriteLine("打开绿灯操作失败");
        //                               break;
        //                       }
        //                   }
        //               }
